...
interface PatientEducationManagerProps {
  ...
  onAddMaterial: (file: File, description: string) => void;
...
const PatientEducationManager: React.FC<PatientEducationManagerProps> = ({ department, onAddMaterial, onDeleteMaterial, onUpdateMaterialDescription, onAddPatient, onDeletePatient, onSendMessage, onBack, supabase }) => {
    ...
    const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        onSendMessage(selectedPatient.id, { file: file }, 'manager');
                        ...
                        <div className="flex flex-col flex-grow min-h-0">
                            ...
                            <div className="flex-grow overflow-y-auto p-4 space-y-4 bg-slate-50 dark:bg-slate-900/50">
                                ...
                                <div key={msg.id} className={`flex items-end gap-2 ${msg.sender === 'manager' ? 'justify-end' : 'justify-start'}`}>
                                    ...
                                    {msg.file && (
                                        <button 
                                        onClick={() => setPreviewMaterial({ id: msg.id, name: msg.file!.name, type: msg.file!.type, filePath: msg.file!.path })}
...
const ContentManager: React.FC<{
    ...
    const handleSaveMaterialWithDescription = () => {
        if (pendingFile) {
            ...
            onAddMaterial(file, materialDescription.trim());

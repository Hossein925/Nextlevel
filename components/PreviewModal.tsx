...
const PreviewModal: React.FC<PreviewModalProps> = ({ isOpen, onClose, material, supabase }) => {
  ...
  const renderContent = () => {
    ...
    if (type.startsWith('image/')) {
      return <img src={fileUrl} alt={name} className="max-w-full max-h-full object-contain" />;
    }
    ...
    if (type === 'application/pdf') {
        return <iframe src={fileUrl} className="w-full h-full" title={name} />;
    }
        ...
        <div className="flex justify-between items-center border-b border-slate-200 dark:border-slate-700 p-4 sticky top-0 bg-white dark:bg-slate-800 z-10">
          ...
          <div className="flex items-center gap-2">
            <a
                ...
                className={`inline-flex items-center gap-2 px-3 py-1.5 text-sm font-semibold text-slate-700 bg-slate-100 rounded-md hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600 ${!fileUrl || isLoading ? 'opacity-50 pointer-events-none' : ''}`}
            >
